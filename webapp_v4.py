...
def make_downloadable_df_format(data, format_type="csv"):
    """
    Save the DataFrame in the specified format (CSV, JSON, or XLSX) in the current project directory,
    display the JSON data on the webpage if the format is JSON, and provide a download link.
    
    Args:
        data (pd.DataFrame): The DataFrame to be saved.
        format_type (str): The format to save the data in ("csv", "json", or "xlsx").
    """
    if format_type == "csv":
        ...
        new_filename = f"fake_dataset_{timestr}.json"
        file_path = os.path.join(os.getcwd(), new_filename)
        with open(file_path, "w") as f:
            f.write(datafile)
        st.json(datafile)  # Display JSON data on the webpage
    elif format_type == "xlsx":
        new_filename = f"fake_dataset_{timestr}.xlsx"
        file_path = os.path.join(os.getcwd(), new_filename)
        data.to_excel(file_path, index=False, engine='openpyxl')
    else:
        st.error("Unsupported format type")
        return
    ...
    if format_type in ["csv", "json"]:
        ...
        href = f'<a href="data:application/vnd.openxmlformats-officedocument.spreadsheetml.sheet;base64,{b64}" download="{new_filename}">Click Here to Download</a>'
        st.markdown(href, unsafe_allow_html=True)
...
def main():
    ...
    if choice == "Home":
        ...
        dataformat = st.sidebar.selectbox("Save Data As", ["csv", "json", "xlsx"])
